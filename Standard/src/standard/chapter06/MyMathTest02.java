...
//클래스 메서드(static메서드)와 인스턴스 메서드 + 설명

//클래스 메서드도 클래스 변수처럼 객체를 생성하지 않고, [클래스명].[메서드명(매개변수)] 로 호출할 수 있다
//반면에 인스턴스 메소드는 반드시 객체를 생성해야 한다.

/*
 * 클래스변수와 인스턴스변수, 클래스 메서드와 인스턴스 메서드를 정리해보자
 * 
 ...
 * 	클래스 메서드는 인스턴스 생성없이 호출 가능하므로 클래스 메서드가 호출되었을 때 인스턴스가 존재하지 않을 수도 있다.
 ...
 * 	반대로 인스턴스 변수를 필요로 하지 않는다면 static을 붙이자. 메서드 호출시간이 짧아지므로 성능이 향상된다.
...
class MyMath2 {
	...
	//인스턴스 메서드, 인스턴스변수 a,b만을 이용해서 작업하므로 매개변수가 필요없다.
	long add() 		{return a+b;}	// a, b는 인스턴스 변수
	long subtract() {return a-b;}
	long multiply() {return a*b;}
	double divide() {return a/b;}
	
	//클래스 메서드, 인스턴스변수와 관계없이 매개변수만으로 작업이 가능하다. -> 인스턴스 생성 안해도 됨
...
class MyMathTest02 {
	public static void main(String[] args) {
		...
		//클래스메서드 호출, 인스턴스 생성없이 직접 호출 가능. 
		//인스턴스가 필요 없다는 걸 보여주기 위해 먼저 작성함. 
